...
local capList = {"sunHomeAngles", "sunPosition", "sunTimes",
                 "sunSeasons", "sunPartsOfTheDay"}
...
--    log.info( string.format( "getTimes from device %s=%s   %s", k, field, config.date.toString(t[field]) ) )
  ...
  for sensor, state in pairs( positionSensors ) do
    device.profile.components["SunPosition"]:emit_event( cap.sunPosition[sensor]( state, eventVis ) )
  end
...
--          north = config.ternary((270.0 <= azs) or  (azs <  90.0), azsNorth,  0,
  ...
  for sensor, state in pairs( angleSensors ) do
    device.profile.components["SunHomeAngles"]:emit_event( cap.sunHomeAngles[sensor]( state, eventVis ) )
  end
...
--    device:emit_event( cap.partsOfTheDay1[sensor]( state, eventVis ) )
    device.profile.components["SunPartsOfTheDay"]:emit_event( cap.sunPartsOfTheDay[sensor]( state, eventVis ) )
  end
end
-----------------------------------------------------------
...
--  local eventVis = { visibility = { displayed = false} }
  ...
  device.profile.components["SunTimes"]:emit_event( cap.sunTimes.dayLength( dayLength ) )
...
--    log.info( string.format( "season = %s, start=%s, percentage=%6.2f", sensor, day, state ) )
    device.profile.components["SunSeasons"]:emit_event( cap.sunSeasons[sensor]( state, eventVis ) )
  end
end
-----------------------------------------------------------
