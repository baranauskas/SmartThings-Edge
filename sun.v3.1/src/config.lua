...
function config.date.getDate( osTime )
  local date = os.date("!*t", osTime )
  return date.year, date.month, date.day
end

function config.date.weekOfMonth( osTime )
 local year, month, day = config.date.getDate( osTime )
 local first = os.time( {year=year, month=month, day=1} )
 local firstDay =  os.date("!*t", first ).wday
 return math.ceil( (day + (firstDay - 1)) / 7 )
end

function config.date.weekOfYear( osTime )
 return math.ceil( os.date("!*t", osTime ).yday / 7 )
end
